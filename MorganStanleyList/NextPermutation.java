...
public class NextPermutation {
    ...
    public void nextPermutation(int[] nums) {
        ...
        int breakPoint=-1;  // it points to index where arr[i]<arr[i+1] is found. breakPoint points to arr[i].
        for(int i=nums.length-2;i>=0;i--)
        {
            if(nums[i]<nums[i+1])   // watch TUF channel solution for this intuition.
            {
                breakPoint = i;
                break;
            }
        ...
        if(breakPoint==-1)  // means array is sorted in descending order. so reverse the array which si our soln
            reverse(nums,0,nums.length-1);
        
        else    // found breakpoint where arr[i] < arr[i+1]
        {
            // now we find on the right side of the breakpoint, smallest element which is greater than element at breakpoint.
            // we exchange these 2 ele.
            ...
            // reverse the list after swapping ele above and reverse from (breakpoint+1 to end of arr).
