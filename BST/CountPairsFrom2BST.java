 ...
 * We will be given 2 BSTs and a number, we need to count pairs whose sum equal to given sum.
 * We do inorder traversal of one bst and reverse inorder traversal of other bst.
 */
package BST;
...
public class CountPairsFrom2BST {
...
static int countPairs(Node root1, Node root2,int x)
{
    ...
    // inorder traversal of BST 2. We move to rightmost end as in bst-1 we move to left end, we add smallest and largest elements in respective bsts
    while (root2 != null) {
    st2.push(root2);
    root2 = root2.right;
    }

    // if either of the stack gets empty then corresponding
    ...
    // inorder traversal of BST 1. If the sum of 2 tops is < x means we need to move to next element in stack-1 bcz stack-2 already points to largest element only, nut stk-1 points to smallest element. So increase it.
    ...
    // move to next possible node in the
    // reverse inorder traversal of BST 2. Same as above case.
    else {
    st2.pop();
    root2 = top2.left;
    }
